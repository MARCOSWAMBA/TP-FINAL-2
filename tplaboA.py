...
class MiVentana(QMainWindow):
    def __init__(self):
        super().__init__()
        uic.loadUi("agenda-1/d-agenda.ui", self)
        ...
        # Conexion de BD
        self.conexion = sqlite3.connect("agenda-1/usuarios")
    ...
    def on_aceptar(self):
        ...
        if self.flag == "editar":
            ...
            # Recorremos los labels y editamos las listas
            ...
            formatoDeLaLista = [indexUltimate, labels[0].text(), labels[1].text(), labels[2].text(), labels[3].text(), labels[4].text(), labels[5].text(),labels[6].text(),labels[7].text(), labels[7].text()]
    ...
    def on_cargarBD(self):
        # Seleccionador de usuarios dentro de la base de datos
        ...
        for usuario in usuarios:
            ...
            formatoDeLaLista = [usuario[0], usuario[1], usuario[2], usuario[3], usuario[4], usuario[5],usuario[6],usuario[7], usuario[8], self.status]
            self.lista.addItem(formatoDeLaListaQt)
            self.baseDatos.append(formatoDeLaLista)
    def on_click(self):
        ...
        # Recorremos los labels para ir rellenarlos con los datos -
        # de la lista BaseDatos
        for num, label in enumerate(labels):
    ...
    def on_eliminar(self):
        ...
        self.msg.setText(f'¿Intentas eliminar el usuario {click[0]} {click[1]} {click[2]}')
    ...
    def on_guardar(self):
        for i in self.baseDatos:
            if i[9] == 'delete':
                ...
                self.cursor.execute(f"insert into usuarios (nombre, apellido, mail, telefono, direccion, nacimiento, altura, peso) values ('{i[1]}','{i[2]}','{i[3]}','{i[4]}','{i[5]}','{i[6]}','{i[7]}','{i[8]}')")
                self.conexion.commit()
                print('AGREGADO')
            elif i[9] == 'edit':
